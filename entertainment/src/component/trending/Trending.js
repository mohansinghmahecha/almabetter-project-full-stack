...
// API key and URL for fetching trending movies from the TMDB API
const API_KEY = process.env.REACT_APP_MYAPI;
const API_URL = `https://api.themoviedb.org/3/trending/movie/week?api_key=${API_KEY}`;

export default function Trending() {
  const nav = useNavigate(); // Initialize useNavigate hook for navigation
  const [movies, setMovies] = useState([]);

  function trandingOnClick(id) {
    nav(`/movie-detail/${id}`); // Navigate to the movie detail page with the customized movie's ID
  }
  // Function to handle saving a movie to local storage
  ...
  // useEffect hook to fetch the trending movies when the component mounts
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        const response = await fetch(API_URL);
        const data = await response.json();
        setMovies(data.results.slice(0, 5));// Update the state with the first 5 trending movies
      } catch (error) {
